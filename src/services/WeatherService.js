class WeatherService {
    ...
    getDailyWeatherData = async (city) => {
        const weatherData = await this.getResource(
            `https://api.openweathermap.org/data/2.5/weather?&units=metric&appid=b2714a5cbe36acd4448f2292ecddf4ed&q=${city}`
        );

        return this.transformDailyData(weatherData);
    }

    transformDailyData = (weatherData) => {
        return {
            id: weatherData.id,
    ...
    transformWeekData = (weatherData) => {
        ...
        const workList = this.getWorkList(weatherData, todayDate, lastDate);

        const days = this.getReducedWorkList(workList);
    ...
    getReducedWorkList = (workList) => {
        return workList.reduce((acc, current) => {
    ...
    getWorkList = (weatherData, todayDate, lastDate) => {
